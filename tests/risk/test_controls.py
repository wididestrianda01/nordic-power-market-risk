...
def test_risk_state_enforces_three_day_cooldown_and_drawdown_recovery() -> None:
    ...
    # Drawdown (120) stays below the 20x daily-limit drawdown ceiling (2000).
...
def test_drawdown_limit_triggers_only_above_multiplier() -> None:
    state = RiskState()
    state.observe(realized_loss_eur=2500.0, loss_limit_eur=100.0, observed_on=date(2025, 1, 1))

    assert state.drawdown_eur == pytest.approx(2500.0)
